...
float rectangle(vec2 frag_coord,vec2 size){
    ...
    float outside=length(max(componentDistance,0.));
    float inside=min(max(componentDistance.x,componentDistance.y),0.);
...
void main(){
    ...
    float rect_sdf=rectangle(rect_coord,vec2(.1,.3));
    
    vec2 circle_coord=coord_N;
    circle_coord=circle_coord-vec2(.5,.5);
    circle_coord=rotate(circle_coord,u_time);
    circle_coord-=vec2(0.,.4);
    
    float circle_sdf=circle(circle_coord,.2);
    float combined_sdf=min(rect_sdf,circle_sdf);
    // try min
    
    float fill=step(0.,combined_sdf);
    float stroke=step(-.01,combined_sdf)-step(0.,combined_sdf);
