...
void main(){
    ...
    // horizontal box blur
    // averaging the pixel and it's neighbors
    blur+=texture2D(u_sampler,uv-vec2(3./u_resolution.x,0.));
    blur+=texture2D(u_sampler,uv-vec2(2./u_resolution.x,0.));
    blur+=texture2D(u_sampler,uv-vec2(1./u_resolution.x,0.));
    blur+=texture2D(u_sampler,uv);
    blur+=texture2D(u_sampler,uv+vec2(1./u_resolution.x,0.));
    blur+=texture2D(u_sampler,uv+vec2(2./u_resolution.x,0.));
    blur+=texture2D(u_sampler,uv+vec2(4./u_resolution.x,0.));
    blur/=7.;
    ...
    gl_FragColor=mix(color,blur,step(.5,uv.x));
}