...
async function main() {
  ...
  const vertex_shader_source = await fetch(canvas.dataset.vert).then(r =>
    r.text()
  );
  console.log(vertex_shader_source);

  const fragment_shader_source = await fetch(canvas.dataset.frag).then(r =>
    r.text()
  );
  ...
  const texture = loadTexture(
    gl,
    "http://compform.net/experimental_shaders/textures/textures/pear.jpg",
    gl.MIRROR_REPEAT,
...
function loadTexture(gl, url, wrap, min_filter) {
  wrap = wrap || gl.REPEAT; //   check to see if wrap was passed in, if not it is undefined
  ...
  //   uploads texture data to tetxture buffer on video card
  gl.texImage2D(
    gl.TEXTURE_2D,
    level,
    internalFormat,
  ...
  image.onload = function() {
    ...
    // textures want to be power of 2 - best path for webgl textures
  ...
  //   ask permission for cross-site image sharing/grabbing
  image.crossOrigin = "";
  image.src = url;
